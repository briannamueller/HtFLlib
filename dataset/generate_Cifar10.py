#!/usr/bin/env python
"""Create CIFAR-10 client splits and store each configuration in a descriptive directory."""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Build CIFAR-10 client splits with descriptive folders.")
...
def main():
    ...
    if config_path.exists():
        print(
            f"\nDataset already generated at {dataset_dir} (partition_id={partition_id}).\nUse --dataset {args.dataset_name} --data-partition {partition_id} when running the experiment."
        )
        return
