...
#   Runs: cd ${REPO_ROOT}/system && python main.py ... --phase {1|2|3} --ckpt_root ...
#
...
# Minimal PYTHONPATH: project root only; main.py will handle anything else.
...
case "${PHASE}" in
  base)
    ...
    cd "${REPO_ROOT}/system"
    python -u main.py ${COMMON_ARGS} ${base_args} \
      --phase 1 \
      --ckpt_root "${ckpt_root}" \
  ...
  graph)
    ...
    mkdir -p "${outputs_root}"
    cd "${REPO_ROOT}/system"
    python -u main.py ${COMMON_ARGS} ${base_args} ${graph_args} \
      --phase 2 \
      --ckpt_root "${ckpt_root}" \
      --outputs_root "${outputs_root}"
  ...
  meta_grid)
    ...
    mkdir -p "${outputs_root}"
    python -X faulthandler -u main.py ${COMMON_ARGS} ${base_args} ${graph_args} ${gnn_args} \
      --phase 3 \
      --ckpt_root "${ckpt_root}" \
      --outputs_root "${outputs_root}"
    ;;
  ...
  pae)
    ...
    mkdir -p "${outputs_root}"
    cd "${REPO_ROOT}/system"
    python -u main.py ${COMMON_ARGS} ${base_args} ${pae_args} \
      --phase 2 \
      --ckpt_root "${ckpt_root}" \
      --outputs_root "${outputs_root}"
    ;;
